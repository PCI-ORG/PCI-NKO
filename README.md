...
3. [Data Preparation and Model Training](#3-data-preparation-and-model-training)
    - [Data Preparation](#21-data-preparation)
    - [Model Training](#22-model-training)
...
**Note:** The training process requires significant disk space, primarily for generated logs. Depending on the size of your source data and training settings, the required disk space may vary considerably. Please allocate at least 10GB of free disk space, or adjust the logging configuration according to your training needs.
...
**Note:**OpenRouter provides a platform to easily try various open-source and closed-source large language models. Registration is recommended. If you want to use a different large language model, OpenRouter is compatible with the OpenAI python library, minimizing necessary code changes. If using a non-OpenAI model (e.g., Anthropic), you'll need to install new packages and modify the API call in `analysis_util.py`.
...
This will confirm whether you're using CUDA, Apple's Metal Performance Shaders (MPS), or CPU. CPU is strongly discouraged for model training. If the result is not CUDA or MPS, please check your PyTorch installation.

**Note:** If CUDA devices are available, uncomment the last line in the .env file.

**Alternative Setup Using pyenv or venv:**
...
``` 
After this, proceed with steps 6-8 as described above to complete your setup.
...
1. Create a /data folder in the root directory and place the raw data file (eg. `demoset.csv`) inside it.
...
For example, while the source file might contain articles from 2018-01-01 to 2024-02-01, you can choose to study a specific period, such as 2022-01-01 to 2024-02-01. This flexibility allows for focused analysis on specific time periods, especially useful when dealing with datasets spanning long periods (e.g., PCI-China covers nearly 80 years of data).

The script will generate a directory starting with "whole_sets_" in the /data/processed directory, containing three subdirectories: "test", "train", and "validate". Articles are divided into 20-day blocks based on their publication date, and within each block, randomly assigned to train, validate, and test subsets in a 14:3:3 ratio. This division schema can be modified in `corpus_preparation.py`.
...
**Note:** This script typically needs to be run only once at the beginning of the project, as it doesn't affect the subsequent workflow.
